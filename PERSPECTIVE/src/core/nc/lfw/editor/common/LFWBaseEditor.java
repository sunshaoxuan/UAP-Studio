...
//		super.createPartControl(parent);
		...
		item1.setText(M_lfw_core.LFWBaseEditor_4);
	...
	protected TreeItem getSelectedTreeItem(String modelRootName){
		...
		if (input instanceof LfwBaseEditorInput) {
			...
			if(projectTreeItem == null){
				throw new LfwRuntimeException(M_lfw_core.LFWBaseEditor_0);
			}
			...
			if(dirTreeItem == null){
				throw new LfwRuntimeException(M_lfw_core.LFWBaseEditor_1);
			}
		...
		if (input instanceof ElementEditorInput) {
			...
			try{
				...
				if(selectedTI != null && !selectedTI.isDisposed()){
					...
					this.setDirtyFalse();
					MessageDialog.openError(null, M_lfw_core.LFWBaseEditor_2, M_lfw_core.LFWBaseEditor_3);
//					this.dispose();
					MainPlugin.getDefault().getWorkbench().getActiveWorkbenchWindow().getActivePage().closeEditor(this, true);
//					MainPlugin.getDefault().getWorkbench().getActiveWorkbenchWindow().getActivePage().hideEditor(this);
					return;
				}
			}catch(Exception e){
	...
	public LFWSeparateTreeItem getWebSeparateTreeItem(String type){
		...
		if(widgetTreeItem != null){
			...
			if(WEBPersConstants.COMPONENTS.equals(type)){
				type = LFWTool.getWEBProjConstantValue("COMPONENTS"); //$NON-NLS-1$
			}else if(WEBPersConstants.DATASET.equals(type)){
				type = LFWTool.getWEBProjConstantValue("DATASET"); //$NON-NLS-1$
			}else if(WEBPersConstants.REFNODE.equals(type)){
				type = LFWTool.getWEBProjConstantValue("REFNODE"); //$NON-NLS-1$
			}else if(WEBPersConstants.COMBODATA.equals(type)){
				type = LFWTool.getWEBProjConstantValue("COMBODATA"); //$NON-NLS-1$
			}
			...
			jump:for (int i = 0; i < separasTreeItems.length; i++) {
				...
				if(item instanceof LFWSeparateTreeItem){
					...
					if(seitem.getText().equals(LFWTool.getWEBProjConstantValue("CONTEXT"))){ //$NON-NLS-1$
...
//	}
	...
	public static LFWBaseEditor getActiveEditor() {
		...
		if (editor == null){
			...
			LFWExplorerTreeView view = LFWExplorerTreeView.getLFWExploerTreeView(null);
			if(item instanceof LFWBasicTreeItem && ((LFWBasicTreeItem)item).getEditorInput()!=null){
				editor = view.getSite().getPage().findEditor(((LFWBasicTreeItem)item).getEditorInput());
				if(editor != null && editor instanceof LFWBaseEditor)
					return (LFWBaseEditor) editor;
				else if(editor instanceof WindowEditor){
					return (LFWBaseEditor)((WindowEditor)editor).getSelectedPage();
				}
			}
			if(item instanceof PortalBasicTreeItem && ((PortalBasicTreeItem)item).getEditorInput()!=null){
				editor = view.getSite().getPage().findEditor(((PortalBasicTreeItem)item).getEditorInput());
				if(editor != null && editor instanceof LFWBaseEditor)
					return (LFWBaseEditor) editor;
			}
		}
