...
public class DsRelationSetDialog extends DialogWithTitle {
	...
	private class ModifyFiedRelation extends Command{
		public ModifyFiedRelation(){
			super(M_perspective.DsRelationSetDialog_0);
		}
	...
	protected void okPressed() {
		...
		if(fieldRelationid == null || fieldRelationid.equals("")){ //$NON-NLS-1$
			MessageDialog.openError(null, M_perspective.DsRelationSetDialog_1, M_perspective.DsRelationSetDialog_2);
			return;
		}
		...
		if(items != null && items.length > 0 && itemsright.length > 0){
			...
			for (int i = 0; i < itemsright.length; i++) {
				...
				if(rightitem.getIsmatch().equals("Y")){ //$NON-NLS-1$
					size ++;
				}
			}
			if(size != 1){
				MessageDialog.openError(null, M_perspective.DsRelationSetDialog_1, M_perspective.DsRelationSetDialog_3);
				return;
			}
			...
			if(dsElement instanceof DatasetElementObj && refDsElement instanceof RefDatasetElementObj){
				...
				if(ds.getFrom() != null){
					MessageDialog.openError(null, M_perspective.DsRelationSetDialog_4, M_perspective.DsRelationSetDialog_5);
					return;
				}
				...
				for (int i = 0; i < itemsright.length; i++) {
					DSRelationField rightitem = (DSRelationField)itemsright[i];
					if(rightitem.getIscontains().equals("Y") && rightitem.getIsmatch().equals("Y")){ //$NON-NLS-1$ //$NON-NLS-2$
						MatchField matchField = new MatchField();
						matchField.setIscontains("Y"); //$NON-NLS-1$
						matchField.setIsmacth("Y"); //$NON-NLS-1$
						matchField.setReadField(rightitem.getId());
						matchField.setWriteField(field.getId());
						matchFieldList.add(matchField);
					}
					//选中并且不是matchfield
					if(rightitem.getIscontains().equals("Y") && rightitem.getIsmatch().equals("N")){ //$NON-NLS-1$ //$NON-NLS-2$
						if(rightitem.getMatchfield() != null && !rightitem.getMatchfield().equals("")){ //$NON-NLS-1$
							MatchField matchField = new MatchField();
							matchField.setReadField(rightitem.getId());
							...
							Field copyField = dsElementnew.getDs().getFieldSet().getField(field.getId() + "_" + rightitem.getId()); //$NON-NLS-1$
							if(copyField == null){
								copyField = new Field();
								copyField.setId(field.getId() + "_" + rightitem.getId()); //$NON-NLS-1$
				...
				if(!match){
					MessageDialog.openError(Display.getCurrent().getActiveShell(), M_perspective.DsRelationSetDialog_6, M_perspective.DsRelationSetDialog_7);
					return;
				}
...
//					listener.setLocation(new org.eclipse.draw2d.geometry.Point(pointX, pointY));
					...
					if(dsElementnew.getDs().getFieldRelations() != null){
						FieldRelation oldrelation = ((RefDatasetElementObj)refDsElement).getRefFieldRelation();
						if(oldrelation!=null){
							dsElementnew.getDs().getFieldRelations().removeFieldRelation(oldrelation.getId());
							oldrelation.setId(fieldRelationid);
						}
...
//						if(	fr.getRefDatasetid().equals(sourcenew.getDs().getId()))
//							parentfr = fr;
						if(fr.getRefDatasetid().equals(targetnew.getDs().getId()))
							frs.removeFieldRelation(fr.getId());
//						fr.getRefDatasetid()
//						FieldRelation fr = frss[i];
...
//					}
					...
					if(rightitem.getIscontains().equals("Y") && rightitem.getIsmatch().equals("N")){ //$NON-NLS-1$ //$NON-NLS-2$
						if(rightitem.getMatchfield() != null && !rightitem.getMatchfield().equals("")){ //$NON-NLS-1$
							...
							copyField.setId(dsobj.getDs().getId() + "_" + field.getId() +  "_" + rightitem.getId()); //$NON-NLS-1$ //$NON-NLS-2$
							copyField.setField(dsobj.getDs().getId() + "_" + field.getId() + "_" + rightitem.getId()); //$NON-NLS-1$ //$NON-NLS-2$
							//向ds中添加字段
			...
			//	WhereField[] whereFieldss = whereFieldList.toArray(new WhereField[whereFieldList.size()]);
				...
				childfr.setExtendAttribute("parentDs", dsElementnew.getDs().getId()); //$NON-NLS-1$
				
				frs.addFieldRelation(childfr);
				parentds.getDs().setFieldRelations(frs);
...
//				parentfr.addChildRelation(childfr);
			...
			MessageDialog.openConfirm(this.getShell(), M_perspective.DsRelationSetDialog_4, M_perspective.DsRelationSetDialog_8);
			tableleft.setFocus();
			//tableleft.clearAll();
			//tableright.clearAll();
		}
	...
	public void addExtendedCombdatas(ComboData[] comboDatas){
		...
		for (int i = 0; i < separasTreeItems.length; i++) {
			...
			if(item instanceof LFWSeparateTreeItem){
				...
				if(seitem.getText().equals(M_perspective.DsRelationSetDialog_9)){
		...
		if(lfwSeparaTreeItem != null){
			...
			for (int i = 0; i < comboDatas.length; i++) {
				if(!list.contains(comboDatas[i].getId())){
					...
					new LFWComboTreeItem(lfwSeparaTreeItem, comboData, M_perspective.DsRelationSetDialog_10);
				}
			}
		}		
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		new TableColumn(tableleft,SWT.NONE).setText("ID"); //$NON-NLS-1$
		layoutleft.addColumnData(new ColumnWeightData(150));
		//Field字段
		new TableColumn(tableleft, SWT.NONE).setText("Field"); //$NON-NLS-1$
		...
		//Field字段
		new TableColumn(tableright, SWT.NONE).setText(M_perspective.DsRelationSetDialog_12);
		layoutright.addColumnData(new ColumnWeightData(100));
		new TableColumn(tableright,SWT.NONE).setText(M_perspective.DsRelationSetDialog_13);
		layoutright.addColumnData(new ColumnWeightData(150));
		new TableColumn(tableright,SWT.NONE).setText(M_perspective.DsRelationSetDialog_14);
		//选择匹配字段
		layoutright.addColumnData(new ColumnWeightData(150));
		new TableColumn(tableright, SWT.NONE).setText(M_perspective.DsRelationSetDialog_15);
		...
		if(refDsElement instanceof RefDatasetElementObj){
			...
			if(originalfs != null){
				for (int i = 0; i < originalfs.length; i++) {
					...
					dsrf.setIscontains("Y"); //$NON-NLS-1$
					dsrf.setIsmatch("N"); //$NON-NLS-1$
					rightinput.add(dsrf);
				}
			}
		...
		String[] names = {"Y","N"}; //$NON-NLS-1$ //$NON-NLS-2$
		...
		tvright.setColumnProperties(new String[]{"ID", "Field","ismatch","isinput","matchField"}); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
				...
				if(fieldRelations != null){
					for (int i = 0; i < fieldRelations.length; i++) {
						...
						if(fieldrelation.getRefDataset().equals(targetnew.getDs().getId())){
							if(targetnew.getRefFieldRelation() == null)
								return;
								...
								String fieldKey = whereField.getKey();
									for (int k = 0; k < righttableItems.length; k++) {
										if(((DSRelationField)righttableItems[k].getData()).getId().equals(fieldKey)){
											DSRelationField dsFieldRelation = (DSRelationField) righttableItems[k].getData();
											dsFieldRelation.setIsmatch("Y"); //$NON-NLS-1$
											dsFieldRelation.setIscontains("N"); //$NON-NLS-1$
											readFields.add(dsFieldRelation);
											tvright.update(dsFieldRelation, null);
											break;
											}
	...
	private FieldRelation getChildRelation(DatasetElementObj source, RefDatasetElementObj target){
		...
		if(frs != null){
			...
			for (int i = 0; i < frrs.length; i++) {
				if(frrs[i].getRefDatasetid().equals(target.getDs().getId()))
					return frrs[i];
