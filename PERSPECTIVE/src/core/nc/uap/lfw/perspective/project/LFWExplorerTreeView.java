...
import nc.uap.lfw.editor.extNode.FuncBrowserEditor;
...
import nc.uap.lfw.listview.core.ListViewEditorInput;
import nc.uap.lfw.pagination.PaginationEditor;
import nc.uap.lfw.pagination.PaginationEditorInput;
...
import nc.uap.lfw.perspective.webcomponent.LFWMethodTreeItem;
	...
	public void createPartControl(Composite parent) {
		...
		tree.addListener(SWT.Expand, new Listener() {
			...
			public void handleEvent(Event event) {
				...
				if (root instanceof LFWProjectTreeItem) {
					((LFWProjectTreeItem) root).expandTree();
				}
...
//				LFWPersTool.getCurrentProject());
		...
		if (widgetlist.length > 0) {
			...
			for (int i = 0; i < widgetlist.length; i++) {
				...
				if(widget==null||widget.getId()==null){
					MainPlugin.getDefault().logError("view id:"+widgetlist[i].getId()+M_perspective.LFWExplorerTreeView_5);   //$NON-NLS-1$
					continue;
				}			
				...
				LFWWidgetTreeItem widgetTreeItem = new LFWWidgetTreeItem(ti, widgetfile, widget,
						caption+"["+ widget.getId()+ "]"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if(controlName!=null&&!"".equals(controlName)){  //$NON-NLS-1$
			...
			try {
				File classFile = appControl.getLocation().toFile();
				String content = FileUtilities.fetchFileContent(classFile, "GBK");
...
//						method.getStartPosition()
						LFWMethodTreeItem mItem = new LFWMethodTreeItem(metadata, classFile, method.getName().toString()+"()");
						mItem.setPositon(method.getStartPosition());
						mItem.setLength(method.getLength());
					}
				}
...
//		}
	...
	public void refreshAppItem() {
		...
		if(app.getWindowList().size()>0){
			for(WindowConfig pm: app.getWindowList()){
				String id = pm.getId();
				LfwWindow win = LFWAMCConnector.getWindowById(id);
				if(win!=null){
					String winid = null;
					if(win.getComponentId()==null||win.getComponentId().equals(LfwUIComponent.ANNOYUICOMPONENT)){
						winid = win.getId();
					}
					else winid = win.getComponentId() + "." + win.getId();  //$NON-NLS-1$
					File appFolder = ((LFWApplicationTreeItem)ti).getFile();
					String path = appFolder.getPath().substring(0,appFolder.getPath().indexOf("applications"))+"nodes/"+winid.replace(".", "/");    //$NON-NLS-1$ //$NON-NLS-2$//$NON-NLS-3$ //$NON-NLS-4$
//					String path = win.getFoldPath();					
					File file = new File(path);
					LFWPageMetaTreeItem pmItem = new LFWPageMetaTreeItem(ti, file, win.getCaption() + "[" + win.getId() +"]");   //$NON-NLS-1$ //$NON-NLS-2$
					pmItem.setPm(win);
...
//		paramMap.put(WebConstant.PAGE_ID_KEY, pageIdKey);
	...
	public void dealCurrentWidgetTreeItem(LFWWidgetTreeItem widgetTreeItem) {
		...
		if(widgetTreeItem.getWidget().getProvider()==null)
...
//					.getRefnodeRelations();
			RefNodeRelation[] rels = refnodeRelations.getRefnodeRelations();
...
//		}
		//component组件
		LFWSeparateTreeItem componentD = createFolder(parent, file, LFWTool
		...
		for (int i = 0; i < grids.length; i++) {
			...
			if(((GridComp)grids[i]).getMenuBar()!=null){
				geneGridMenuTreeItem(componentD, ((GridComp)grids[i]).getMenuBar(),grids[i].getId());
			}
		}
		WebComponent[] lvs = widget.getViewComponents().getComponentByType(
				ListViewComp.class);
		for (int i = 0; i < lvs.length; i++) {
			geneWebCompTreeItem(componentD, lvs[i]);
		}
		WebComponent[] pages = widget.getViewComponents().getComponentByType(
				PaginationComp.class);
		for (int i = 0; i < pages.length; i++) {
			geneWebCompTreeItem(componentD, pages[i]);
		}
...
//				WEBPersConstants.CTRL_EN);
		LFWDirtoryTreeItem controllNode = new LFWDirtoryTreeItem(ti,file,M_perspective.LFWExplorerTreeView_2); 
		controllNode.setType(LFWDirtoryTreeItem.METADATA_FOLDER);
	...
	private void geneGridMenuTreeItem(LFWSeparateTreeItem componentD,
			MenubarComp webCompnent,String grid) {
		LFWGridMenubarTreeItem newComp = new LFWGridMenubarTreeItem(
				componentD, M_perspective.LFWExplorerTreeView_4, webCompnent); 
		newComp.setType(LFWWebComponentTreeItem.PARAM_MENUBAR);
		newComp.setGridId(grid);
	}
...
//			newComp.setType(LFWWebComponentTreeItem.PARAM_EXCEL);
		} else if (webCompnent instanceof ListViewComp) {
			LFWWebComponentTreeItem newComp = new LFWWebComponentTreeItem(
					componentD, LFWTool.getWEBProjConstantValue("COMPONENT_LISTVIEW"), webCompnent);  //$NON-NLS-1$
			newComp.setType(LFWWebComponentTreeItem.PARAM_LISTVIEW);
		}else if (webCompnent instanceof PaginationComp) {
			LFWWebComponentTreeItem newComp = new LFWWebComponentTreeItem(
					componentD, LFWTool.getWEBProjConstantValue("COMPONENT_PAGINATION"), webCompnent);  //$NON-NLS-1$
			newComp.setType(LFWWebComponentTreeItem.PARAM_PAGINATION);
	 ...
	 */
	public void openListViewEditor(LFWWebComponentTreeItem listviewComponent) {
		LFWWidgetTreeItem widgetTreeItem = LFWPersTool
				.getWidgetTreeItem(listviewComponent);
		LfwView lfwwidget = widgetTreeItem.getWidget();
		LFWPersTool.setLfwWidget(lfwwidget);
		LFWPageMetaTreeItem pageMetaTreeItem = LFWPersTool
				.getPagemetaTreeItem(listviewComponent);
		LfwWindow pagemeta = null;
		if (pageMetaTreeItem != null)
			pagemeta = pageMetaTreeItem.getPm();
		ListViewComp listview = (ListViewComp) listviewComponent.getData();
		ListViewEditorInput listviewEditorInput = new ListViewEditorInput(listview, lfwwidget,
				pagemeta);
		listviewComponent.setEditorInput(listviewEditorInput);
		String editorid = ListViewEditor.class.getName();
		if (listviewEditorInput == null || editorid == null)
			return;
		IWorkbenchPage workbenchPage = getViewSite().getPage();
		IEditorPart editor = null;
		ListViewComp listviewcomp = (ListViewComp) listviewComponent.getData();
		String editorinputid = null;
		if (pagemeta == null)
			editorinputid = editoridMap.get(lfwwidget.getId() + "listview"  //$NON-NLS-1$
					+ listviewcomp.getId());
		else
			editorinputid = editoridMap.get(pagemeta.getId()
					+ lfwwidget.getId() + "listview" + listviewcomp.getId());  //$NON-NLS-1$
		if (editorinputid != null) {
			IEditorPart[] parts = LFWTool.getEditors(workbenchPage);
			for (int i = 0; i < parts.length; i++) {
				IEditorInput editorInput = (IEditorInput) parts[i]
						.getEditorInput();
				if (editorInput instanceof ListViewEditorInput) {
					ListViewEditorInput lEditorInput = (ListViewEditorInput) editorInput;
					ListViewComp gridnew = (ListViewComp) lEditorInput
							.getCloneElement();
					LfwWindow pmnew = lEditorInput.getPagemeta();
					LfwView widgetnew = lEditorInput.getWidget();
					if (pmnew == null) {
						if (widgetnew.getId().equals(lfwwidget.getId())
								&& gridnew.getId().equals(listviewcomp.getId())) {
							editor = parts[i];
							break;
						}
					} else {
			...
			if (editor != null)
				...
				try {
					workbenchPage.openEditor(listviewEditorInput, editorid);
				} catch (PartInitException e1) {
					MainPlugin.getDefault().logError(e1.getMessage(), e1);
				}
			}
			...
			if (pagemeta == null)
				editoridMap.put(lfwwidget.getId() + "listview" + listviewcomp.getId(),  //$NON-NLS-1$
						lfwwidget.getId() + "listview" + listviewcomp.getId());  //$NON-NLS-1$
			else
				editoridMap.put(pagemeta.getId() + lfwwidget.getId() + "listview"  //$NON-NLS-1$
						+ listviewcomp.getId(), pagemeta.getId()
						+ lfwwidget.getId() + "listview" + listviewcomp.getId());  //$NON-NLS-1$
			try {
				LFWPersTool.getLFWViewSheet();
				workbenchPage.openEditor(listviewEditorInput, editorid);
	 ...
	 * @param listviewComponent
	 */
	public void openPaginationEditor(LFWWebComponentTreeItem paginationComponent) {
		LFWWidgetTreeItem widgetTreeItem = LFWPersTool
				.getWidgetTreeItem(paginationComponent);
		LfwView lfwwidget = widgetTreeItem.getWidget();
		LFWPersTool.setLfwWidget(lfwwidget);
		LFWPageMetaTreeItem pageMetaTreeItem = LFWPersTool
				.getPagemetaTreeItem(paginationComponent);
		LfwWindow pagemeta = null;
		if (pageMetaTreeItem != null)
			pagemeta = pageMetaTreeItem.getPm();
		PaginationComp paginationComp = (PaginationComp)paginationComponent.getData();
		PaginationEditorInput paginationEditorInput = new PaginationEditorInput(paginationComp, lfwwidget,
				pagemeta);
		paginationComponent.setEditorInput(paginationEditorInput);
		String editorid = PaginationEditor.class.getName();
		if (paginationEditorInput == null || editorid == null)
			return;
		...
		String editorinputid = null;
		if (pagemeta == null)
			editorinputid = editoridMap.get(lfwwidget.getId() + "pagination"  //$NON-NLS-1$
					+ paginationComp.getId());
		else
			editorinputid = editoridMap.get(pagemeta.getId()
					+ lfwwidget.getId() + "pagination" + paginationComp.getId());  //$NON-NLS-1$
		if (editorinputid != null) {
			IEditorPart[] parts = LFWTool.getEditors(workbenchPage);
			for (int i = 0; i < parts.length; i++) {
				IEditorInput editorInput = (IEditorInput) parts[i]
						.getEditorInput();
				if (editorInput instanceof PaginationEditorInput) {
					PaginationEditorInput pageInput= (PaginationEditorInput) editorInput;
					PaginationComp pagenew = (PaginationComp) pageInput
							.getCloneElement();
					LfwWindow pmnew = pageInput.getPagemeta();
					LfwView widgetnew = pageInput.getWidget();
					if (pmnew == null) {
						if (widgetnew.getId().equals(lfwwidget.getId())
								&& pagenew.getId().equals(paginationComp.getId())) {
							editor = parts[i];
							break;
						}
					} else {
			...
			if (editor != null)
				...
				try {
					workbenchPage.openEditor(paginationEditorInput, editorid);
				} catch (PartInitException e1) {
					MainPlugin.getDefault().logError(e1.getMessage(), e1);
				}
			}
			...
			if (pagemeta == null)
				editoridMap.put(lfwwidget.getId() + "pagination" + paginationComp.getId(),  //$NON-NLS-1$
						lfwwidget.getId() + "pagination" + paginationComp.getId());  //$NON-NLS-1$
			else
				editoridMap.put(pagemeta.getId() + lfwwidget.getId() + "pagination"  //$NON-NLS-1$
						+ paginationComp.getId(), pagemeta.getId()
						+ lfwwidget.getId() + "pagination" + paginationComp.getId());  //$NON-NLS-1$
			try {
				LFWPersTool.getLFWViewSheet();
				workbenchPage.openEditor(paginationEditorInput, editorid);
...
//		}
	...
	public void openRefNodeRelEditor(LFWRefNodeRelTreeItem refNodeRel) {
		...
		String editorinputid = null;
		if (pm == null)
			editorinputid = editoridMap.get(lfwwidget.getId() + "refnodrel" + relcomp.getId());  //$NON-NLS-1$
		else
			editorinputid = editoridMap.get(pm.getId() + lfwwidget.getId() + "refnodrel"  //$NON-NLS-1$
					+ relcomp.getId());
		if (editorinputid != null) {
			IEditorPart[] parts = LFWTool.getEditors(workbenchPage);
			for (int i = 0; i < parts.length; i++) {
	...
	public void openToolbarEditor(LFWWebComponentTreeItem toolbarComponent) {
		...
		String editorinputid = null;
		if (pm == null)
			editorinputid = editoridMap.get(lfwwidget.getId() + "toolbar" + toolbar.getId());  //$NON-NLS-1$
		else
			editorinputid = editoridMap.get(pm.getId() + lfwwidget.getId() + "toolbar"  //$NON-NLS-1$
					+ toolbar.getId());
		if (editorinputid != null) {
			IEditorPart[] parts = LFWTool.getEditors(workbenchPage);
			for (int i = 0; i < parts.length; i++) {
	 ...
	 * @param treeItem
	 */
	public void openGridMenubarEditor(LFWGridMenubarTreeItem treeItem) {
		...
		String gridId = treeItem.getGridId();
		menubar = ((GridComp)widget.getViewComponents().getComponent(gridId)).getMenuBar();
//		menubar = widget.getViewMenus().getMenuBar(menubarOld.getId());
		...
		menubarEditorInput.setGridId(treeItem.getGridId());
		String editorid = MenubarEditor.class.getName();
		if (menubarEditorInput == null || editorid == null)
			return;
		treeItem.setEditorInput(menubarEditorInput);
		IWorkbenchPage workbenchPage = getViewSite().getPage();
		IEditorPart editor = null;
...
//	}
	...
	public TreeItem addDirTreeNode(String dirName) throws Exception {
		...
		if (selTIs == null || selTIs.length == 0)
			throw new Exception(M_perspective.LFWExplorerTreeView_6); 
		TreeItem selTI = selTIs[0];
		File parentFile = ((ILFWTreeNode) selTI).getFile();// (File)
		// selTI.getData();
		if (parentFile.isFile())
			throw new Exception(M_perspective.LFWExplorerTreeView_7); 
	...
	public TreeItem addPageMetaTreeNode(String appId, String appName)
		...
		TreeItem[] selTIs = tree.getSelection();
		if (selTIs == null || selTIs.length == 0)
			throw new Exception(M_perspective.LFWExplorerTreeView_8); 
		TreeItem selTI = selTIs[0];
		File parentFile = ((ILFWTreeNode) selTI).getFile();// (File)
		if (parentFile.isFile())
			throw new Exception(M_perspective.LFWExplorerTreeView_9); 
		if (appId != null
			...
			throw new Exception(M_perspective.LFWExplorerTreeView_10 + appId + M_perspective.LFWExplorerTreeView_11);  
...
//		TreeItem selTI = LFWPersTool.getWindowDirectoryTreeItem(project, currentTreeItem);
		TreeItem selTI = currentTreeItem;
	...
	public TreeItem addViewTreeNode(LfwView widget){
		...
		if(!selTI.getExpanded()){
			((LFWDirtoryTreeItem)selTI).expandTree();
			selTI.setExpanded(true);
		}
...
//		selTI.setExpanded(true);
	...
	public TreeItem addDSTreeNode(String dirName,String caption) throws Exception {
		...
		if (originalds != null)
			throw new Exception(M_perspective.LFWExplorerTreeView_12 + dirName + M_perspective.LFWExplorerTreeView_13);  
	...
	public TreeItem addContextMenubarTreeNode(String dirName) throws Exception {
		...
		ContextMenuComp originalds = widget.getViewMenus().getContextMenu(
				dirName);
		if (originalds != null)
			throw new Exception(M_perspective.LFWExplorerTreeView_12 + dirName + M_perspective.LFWExplorerTreeView_14);  
		...
		if (fullName.indexOf(".") != -1) {  //$NON-NLS-1$
			...
			while (leftfolder != null) {
				String newRootPath = rootTreeItem.getFile().getPath().replace(
						M_perspective.LFWExplorerTreeView_15, "web/pagemeta/public/dspool");   //$NON-NLS-2$
...
//	}
	...
	public TreeItem addPoolRefNode(String dirName) throws Exception {
		...
		if (allPoolrefNodeIds.contains(dirName))
			throw new Exception(M_perspective.LFWExplorerTreeView_10 + dirName + M_perspective.LFWExplorerTreeView_16);  
		Tree tree = treeView.getTree();
		TreeItem[] selTIs = tree.getSelection();
		TreeItem selTI = selTIs[0];
		String fullName = dirName;
		...
		if (fullName.indexOf(".") != -1) {  //$NON-NLS-1$
			...
			while (leftfolder != null) {
				String newRootPath = rootTreeItem.getFile().getPath().replace(
						M_perspective.LFWExplorerTreeView_17, "web/pagemeta/public/refnodes");   //$NON-NLS-2$
				File newfile = new File(newRootPath, foldername);
				if (!newfile.exists() || !(newfile.isDirectory())) {
					newfile.mkdir();
				}
	...
	public TreeItem addControlChildTreeNode(String dirName, WebComponent webComponent,String constantName, String type) throws Exception{
		...
		if (oriWebCom != null){
			throw new Exception(M_perspective.LFWExplorerTreeView_12 + dirName + M_perspective.LFWExplorerTreeView_18);  
		}
	 ...
	 * @throws Exception
	 */
	public TreeItem addListViewTreeNode(String dirName) throws Exception {
		return addControlChildTreeNode(dirName, new ListViewComp(), "COMPONENT_LISTVIEW", LFWWebComponentTreeItem.PARAM_LISTVIEW);  //$NON-NLS-1$
	}
	/**
	 * 增加分页条节点
	 * 
	 ...
	 * @throws Exception
	 */
	public TreeItem addPaginationTreeNode(String dirName) throws Exception {
		return addControlChildTreeNode(dirName, new PaginationComp(), "COMPONENT_PAGINATION", LFWWebComponentTreeItem.PARAM_PAGINATION);  //$NON-NLS-1$
	}
...
//	}
	...
	public TreeItem addComboNode(String dirName) throws Exception {
		...
		if (oriComboData != null)
			throw new Exception(M_perspective.LFWExplorerTreeView_12 + dirName + M_perspective.LFWExplorerTreeView_19);  
	...
	public TreeItem addRefNode(String dirName) throws Exception {
		...
		if (originalrefnode != null)
			throw new Exception(M_perspective.LFWExplorerTreeView_20 + dirName + M_perspective.LFWExplorerTreeView_21);  
...
//		refnode.setPagemeta(WEBProjConstants.REFNODE_PAGEMETA);
	...
	public TreeItem addWidgetsTreeNode(String dirName) throws Exception {
		...
		// selTI.getData();
		if (parentFile.isFile())
			throw new Exception(M_perspective.LFWExplorerTreeView_22); 
		File f = new File(parentFile, dirName);
		if (!f.exists()) {
			f.mkdirs();
		}
	...
	public void deleteSelectedTreeNode() throws Exception {
		...
		if (selTI instanceof ILFWTreeNode) {
			...
			if (!MessageDialog.openConfirm(shell, M_perspective.LFWExplorerTreeView_23, M_perspective.LFWExplorerTreeView_24  
					+ selTI.getText() + M_perspective.LFWExplorerTreeView_25)) 
				return;
...
//		selTI.dispose();
	...
	protected void initTree() {
		MessageDialog.openError(null, "error", "fdsaf");
