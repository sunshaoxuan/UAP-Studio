...
//	}
	...
	public void refreshTreeItemText(Dataset ds){
		...
		if(treeItem instanceof LFWDSTreeItem){
			...
			dsTreeItem.setText(ds.getId() + "[" + ds.getCaption() + "]"); //$NON-NLS-1$ //$NON-NLS-2$
		}
...
//		}
	...
	public void setFocus() {
		...
		if(widgetTreeItem != null&&!widgetTreeItem.isDisposed()){
			LFWSeparateTreeItem lfwSeparaTreeItem = getWebSeparateTreeItem(WEBPersConstants.DATASET);
			...
			for (int i = 0; i < childTreeItems.length; i++) {
				...
				if(dsT.getText().indexOf("[") != -1){ //$NON-NLS-1$
					if(dsT.getText().substring(0,dsT.getText().indexOf("[")).equals(ds.getId())){ //$NON-NLS-1$
			...
			for (int i = 0; i < nodesTreeItems.length; i++) {
				...
				if(treeItem.getText().equals(LFWTool.getWEBProjConstantValue("PUBLIC_DATASET"))){ //$NON-NLS-1$
			...
			if(dsId.indexOf(".") != -1){ //$NON-NLS-1$
				String firstFolder = dsId.substring(0, dsId.indexOf(".")); //$NON-NLS-1$
				String leftfolder = dsId.substring(dsId.indexOf(".") + 1); //$NON-NLS-1$
					...
					for (int i = 0; i < childTreeItems.length; i++) {
						...
						if(childTreeItem instanceof LFWDirtoryTreeItem && childTreeItem.getText().equals(firstFolder)){
							if(leftfolder.indexOf(".") != -1){ //$NON-NLS-1$
								firstFolder = leftfolder.substring(0, leftfolder.indexOf(".")); //$NON-NLS-1$
								leftfolder = leftfolder.substring(leftfolder.indexOf(".")+ 1); //$NON-NLS-1$
							}
	 ...
	 public String getPath(){
		if(widgeTreeItem != null){
			...
			String path = file.getPath() + "/widget.wd"; //$NON-NLS-1$
			return path;
		}
		else
			return null;
...
//		dsnewds.setListenerMap(ds.getListenerMap());
		...
		if(ds instanceof IRefDataset){
			if(dsnewds instanceof PubDataset){
				...
				refpubds.setCaption(ds.getCaption());
//				dsobj.setDs(refpubds);
			...
			if(dsnewds instanceof MdDataset){
				...
				refmdds.setCaption(ds.getCaption());
//				dsobj.setDs(refmdds);
			}
//		}else{
//			dsobj.setDs(dsnewds);
			...
			if(currentDS != null && !currentDS.isLazyLoad()){
				...
				if(ecs != null && ecs.length > 0){
					for(EventConf ec : ecs){
						if(ec.getName().equals(DatasetEvent.ON_DATA_LOAD)){
							isAddMethod = false;
							break;
						}
					}
			...
			if(isAddMethod){
				if(widget != null && widget.getControllerClazz() != null && widget.getControllerClazz().trim().length() > 0){
					...
					for(EventConf ec : ecList){
						if(DatasetEvent.ON_DATA_LOAD.equals(ec.getName())){
							eventConf = (EventConf)ec.clone();
							break;
						}
					}
					eventConf.setMethodName(DatasetEvent.ON_DATA_LOAD + "_" + ds.getId()); //$NON-NLS-1$
					String projectPath = LFWAMCPersTool.getLFWProjectPath();
					int index = widget.getControllerClazz().lastIndexOf("."); //$NON-NLS-1$
					...
					String classFilePath = projectPath + File.separator + widget.getSourcePackage() + packageName.replaceAll("\\.", "/"); //$NON-NLS-1$ //$NON-NLS-2$
...
//				if (parent.getText().equals(LFWTool.getWEBProjConstantValue("PUBLIC_DATASET")))
//					break;
...
//				projectPath += "/" + busiCompnent.getText().substring(busiCompnent.getText().indexOf(LFWTool.getWEBProjConstantValue("BUSINESSCOMPONENT")) + 
//						LFWTool.getWEBProjConstantValue("BUSINESSCOMPONENT").length() + 1, busiCompnent.getText().length() -1 );
//			}
//			
...
//		}
	...
	public void RemoveOriginalCombdatas(String id){
		//增加下拉数据集
		LFWSeparateTreeItem lfwSeparaTreeItem = getWebSeparateTreeItem(LFWTool.getWEBProjConstantValue("COMBODATA")); //$NON-NLS-1$
	...
	public void RemoveOriginalRefNodes(String id){
		LFWSeparateTreeItem lfwSeparaTreeItem = getWebSeparateTreeItem(LFWTool.getWEBProjConstantValue("REFNODE")); //$NON-NLS-1$
		if(lfwSeparaTreeItem != null){
			TreeItem[] treeItems = lfwSeparaTreeItem.getItems();
			for (int i = 0; i < treeItems.length; i++) {
				...
				if(comboDatas != null && comboDatas.length > 0){
					for (int i = 0; i < comboDatas.length; i++) {
						if(!list.contains(comboDatas[i].getId())){
							...
							new LFWComboTreeItem(lfwSeparaTreeItem, comboData, LFWTool.getWEBProjConstantValue("COMPONENT_COMBODATA")); //$NON-NLS-1$
						}
					}
				}
			}
	...
	public void addExtendedRefDs(){
		LFWSeparateTreeItem lfwSeparaTreeItem = getWebSeparateTreeItem(LFWTool.getWEBProjConstantValue("DATASET")); //$NON-NLS-1$
		TreeItem[] treeItems = lfwSeparaTreeItem.getItems();
		List<String> list = new ArrayList<String>();
		for (int i = 0; i < treeItems.length; i++) {
	...
	public void addExtendedRefNode(){
		...
		for (int i = 0; i < refnodes.length; i++) {
			if(!list.contains(refnodes[i].getId())){
				...
				new LFWRefNodeTreeItem(lfwSeparaTreeItem, ref, LFWTool.getWEBProjConstantValue("COMPONENT_REFNODE")); //$NON-NLS-1$
			}
		}
	}
...
//		}
	...
	protected void setInput(IEditorInput input) {
		...
		dsElement.setDs(ds);
		setData(ds);
		if(ds instanceof RefMdDataset && ds.getFieldCount()>0)
			caneditflag = false;
		...
		if(widgeTreeItem != null){
			widget = (LfwView)widgeTreeItem.getWidget();
			...
			if(fieldrelations != null){
				...
				if(fieldrelas != null){
					List<FieldRelation> fieldRelist = new ArrayList<FieldRelation>();
					int time = 0;
					while(fieldRelist.size()!=fieldrelas.length&&time<100){
						for(FieldRelation relation:fieldrelas){
							if(relation.getExtendAttributeValue("parentDs")==null&&!fieldRelist.contains(relation)) //$NON-NLS-1$
								fieldRelist.add(relation);
							else{
								String pid = (String)relation.getExtendAttributeValue("parentDs"); //$NON-NLS-1$
								int flag = 0;
								for(FieldRelation exsitRelation:fieldRelist){
									if(exsitRelation.getRefDatasetid().equals(pid)){
										flag = 1;
										break;
									}										
								}
								if(flag == 1&&!fieldRelist.contains(relation)){
									fieldRelist.add(relation);
								}
							}
						}
				...
				for (int i = 0; i < fieldrelas.length; i++) {
					FieldRelation fr = fieldRelist.get(i);
					if(fr.getExtendAttributeValue("parentDs")==null){ //$NON-NLS-1$
						...
						String pid = (String)fr.getExtendAttributeValue("parentDs"); //$NON-NLS-1$
//						parentfr.getref
						List<RefDatasetElementObj> refDataList = graph.getRefds();
						RefDatasetElementObj pobj = null;
						for(RefDatasetElementObj obj:refDataList){
							if(obj.getDs().getId().equals(pid)){
								pobj = obj;
								break;
							}
						}
						Point refpoint = pobj.getLocation();
						Dimension refsize = pobj.getSize();
						String childrefid = fr.getRefDatasetid();
						...
						childrefobj.setDsobj(dsElement);
						childrefobj.setRefFieldRelation(fr);
						pobj.addChild(childrefobj);
						childrefobj.setParent(pobj);
						childrefobj.setSize(new Dimension(100,100));
						childrefobj.setLocation(new Point((refpoint.x + refsize.width + 100),(refpoint.y + 200*0)));
						graph.addRefDs(childrefobj);
						ChildConnection childcon = new ChildConnection(pobj, childrefobj);
						childcon.connect();
					}
...
//					}
	...
	public void doSave(IProgressMonitor monitor) {
		if(ds instanceof IRefDataset&&!caneditflag){
			MessageDialog.openError(null, M_perspective.DataSetEditor_0, M_perspective.DataSetEditor_1);
			setDirtyFalse();
			getCommandStack().markSaveLocation();
			return ;
		}
		...
		if(msg != null){
			String message = M_perspective.DataSetEditor_2+msg;
			if(!MessageDialog.openConfirm(getSite().getShell(), M_perspective.DataSetEditor_0, message))
				return;
		}		
		save();
	}
