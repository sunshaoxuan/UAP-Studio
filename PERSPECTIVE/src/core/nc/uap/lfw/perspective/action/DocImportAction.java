...
public class DocImportAction extends Action{
	
	public DocImportAction(){
		super(M_perspective.DocImportAction_0);
	}
	public void run(){
     File file =openFile();
     if (file == null)
    	 return;
     try {
    	String  path = LFWAMCPersTool.getCurrentProject().getLocation().toOSString() + "/"; //$NON-NLS-1$
		unzipFile(path,file);
	} catch (Exception e) {
