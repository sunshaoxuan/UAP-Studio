...
import nc.uap.lfw.lang.M_lfw_dataset;
 ...
 *
 */
public class MetaDataSelDialog  extends DialogWithTitle {
	...
	public void setMdClassId(String mdClassId) {
		this.mdClassId = mdClassId;
	}
	

	public String getMdClassName() {
		return mdClassName;
	}

	public void setMdClassName(String mdClassName) {
		this.mdClassName = mdClassName;
	}
	...
	public MetaDataSelDialog(Shell parentShell, String title) {
	...
	protected Control createDialogArea(Composite parent) {
		...
		label.setText(M_lfw_dataset.MDDatsetDialog_0);
	...
	protected void okPressed() {
		...
		if(!(treeItem.getData() instanceof MdClassVO)){
			MessageDialog.openError(null, M_lfw_dataset.MDDatsetDialog_1, M_lfw_dataset.MDDatsetDialog_2);
			return;
		}
...
//		else
		...
		fullName = compName + "." + classvo.getName(); //$NON-NLS-1$
		mdClassId = classvo.getId();
		mdClassName = classvo.getDisplayname()!=null?classvo.getDisplayname():classvo.getName();
		super.okPressed();
		...
		public Object[] getChildren(Object parentElement) {
			if(parentElement instanceof MdModuleVO){
				...
				for (int i = 0; i < size; i++) {
					...
					if(mdc.getComponentid()!=null && mdc.getComponentid().equals(component.getId()))
						list.add(mdc);
				}
				return list.toArray();
			}
